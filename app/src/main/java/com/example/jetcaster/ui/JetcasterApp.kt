    ...
    if (appState.isOnline) {
        NavHost(
            ...
            composable(Screen.Player.route) { backStackEntry ->
                /*
                 * 使用viewModel()获取ViewModel实例的目的是为了保障在同一个viewModelStoreOwner中获取到的ViewModel是同一个实例。
                 * 直接通过ViewModel的构造方法来获取无法保障这一点。
                 * viewModel()构造的对象如果有参数，需要自定义一个factory对象，将参数传给factory，然后由factory来生成ViewModel实例。
                 */
